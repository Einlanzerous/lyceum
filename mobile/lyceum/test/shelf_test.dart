...
void main() {
  group('memberStatus', () {
    test('classifies by progress', () {
      ...
      expect(memberStatus(_book(id: 2, progress: 0.4)), MemberStatus.inProgress);
  ...
  group('sortBooks', () {
    final books = [
      _book(id: 1, title: 'Mango', author: 'Clarke', addedAt: '2026-01-02T00:00:00Z'),
      _book(id: 2, title: 'apple', author: 'Adams', addedAt: '2026-03-01T00:00:00Z'),
      _book(id: 3, title: 'Zebra', author: 'Zola', addedAt: '2026-02-01T00:00:00Z'),
    ];

    test('by title, case-insensitive', () {
      final asc = sortBooks(books, const SortState(key: SortKey.title, ascending: true));
      expect(asc.map((b) => b.id), [2, 1, 3]);
    });

    test('by recently added (desc) using added_at', () {
      final desc = sortBooks(books, const SortState(key: SortKey.added, ascending: false));
  ...
  group('buildShelf', () {
    test('rolls a ≥2 series into one item, keeps singletons loose', () {
      final books = [
        _book(id: 1, title: 'Annihilation', series: 'Southern Reach', seriesIndex: 1),
        _book(id: 2, title: 'Authority', series: 'Southern Reach', seriesIndex: 2),
        _book(id: 3, title: 'Dune'),
        _book(id: 4, title: 'Solo', series: 'Lonely', seriesIndex: 1),
      ];
      final items = buildShelf(books, const SortState(key: SortKey.title, ascending: true));
  ...
  group('matchesQuery', () {
    final b = _book(id: 1, title: 'Piranesi', author: 'Clarke', series: 'Standalone');
