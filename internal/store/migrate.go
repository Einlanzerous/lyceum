...
// ascending version order. Each migration runs in its own transaction and is
// recorded in the schema_migrations table. It is idempotent: already-applied
// versions are skipped, so it is safe to call repeatedly.
...
// applyOne runs a single migration's SQL plus its bookkeeping insert inside one
...
// loadMigrations reads and sorts every *.up.sql file from the embedded FS.
